...
def video_to_mp4(
    input, output, fps: int = 0, frame_size: tuple = (), fourcc: str = "mp4v"
):
...
def run(
    ...
    if not source.endswith('.mp4'):
        ...
        subprocess.run(['ffmpeg',
                        ...
                        '-qscale',
                        '0',
                        source.replace(".mov",".mp4"),
                        '-loglevel',
                        'quiet'],shell=True
                      )
        '''
        video_to_mp4(source, source.replace(f".{extension}",".mp4"))
        source = source.replace(f".{extension}",".mp4")
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                features = encoder(im0, t_bboxes)
                
                # DeepSORT -> Storing all the required info in a list.
                if isinstance(t_bboxes, np.ndarray):
                  bbox_tlwh = t_bboxes.copy()
                elif isinstance(t_bboxes, torch.Tensor):
                  bbox_tlwh = t_bboxes.clone()
                bbox_tlwh[:, 0] = t_bboxes[:, 0] - t_bboxes[:, 2] / 2.
                bbox_tlwh[:, 1] = t_bboxes[:, 1] - t_bboxes[:, 3] / 2.
                detections = [Detection(bbox, score, feature) for bbox, score, feature in zip(bbox_tlwh, scores, features) if score > conf_thres]
                ...
                for track in tracker.tracks:
                  ...
                  bbox = track.to_tlwh()
                  d_height, d_width = im0.shape[:2]
                  x, y, w, h = bbox
                  x1 = max(int(x), 0)
                  x2 = min(int(x+w), d_width - 1)
                  y1 = max(int(y), 0)
                  y2 = min(int(y+h), d_height - 1)
                ...
                #Deepsort -> draw boxes for visualization
                if len(outputs) > 0:
                    bbox_xyxy = outputs[:, :4]
        ...
        LOGGER.info(f"{s}{'' if len(det) else '(no detections), '}{t3 - t2:.3f}s")
